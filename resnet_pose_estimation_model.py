...
class ResNet_Pose(nn.Module):
    ...
    def __init__(self, block, layers, rect=None, rect_local=None, num_classes=7, zero_init_residual=False,
        ...
        self.conv_reduce_channels = nn.Conv2d(2560, 512, kernel_size=1, stride=1, padding=0)
        self.upsample = nn.Upsample(size=(7, 7), mode='bilinear', align_corners=True)
        self.avgpool_lp = nn.AvgPool2d(kernel_size=2, stride=2)
        self.rect = rect
        ...
        # 3 branches for Local
        self.conv2_2_1 = nn.Conv2d(512 * block.expansion_al, 64, kernel_size=1)
        self.conv2_2_2 = nn.Conv2d(512 * block.expansion_al, 64, kernel_size=1)
        self.conv2_2_3 = nn.Conv2d(512 * block.expansion_al, 64, kernel_size=1)
    ...
    def _forward_impl(self, x):
      ...
      x_lp_fused = torch.cat([top_left, top_right, bottom_left, bottom_right], dim=1)
      x_lp_fused = self.avgpool_lp(x_lp_fused)
    ...
    #... AP Module
      ...
      x_ap_fused = torch.cat([eye1, eye2, eye_midd, mouth1, mouth2], dim=1)
      x_ap_fused = self.conv_reduce_channels(x_ap_fused)
      x_ap_fused = self.upsample(x_ap_fused)


      x_fused = torch.cat([x_lp_fused, x_ap_fused], dim=1)

      x_al_1 = self.conv2_2_1(x_fused)
      x_al_2 = self.conv2_2_2(x_fused)
      x_al_3 = self.conv2_2_3(x_fused)


      # Apply avgpool to each tensor to reduce spatial dimensions to 1x1
      x_al_fc1 = self.avgpool(x_al_1)     # Resultant shape: [batch_size, 64, 1, 1]
      x_al_fc2 = self.avgpool(x_al_2)     # Resultant shape: [batch_size, 64, 1, 1]
      x_al_fc3 = self.avgpool(x_al_3)     # Resultant shape: [batch_size, 64, 1, 1]


      # Flatten each tensor to prepare for concatenation
      x_al_fc1 = torch.flatten(x_al_fc1, 1)  # Resultant shape: [batch_size, 64]
      x_al_fc2 = torch.flatten(x_al_fc2, 1)  # Resultant shape: [batch_size, 64]
      x_al_fc3 = torch.flatten(x_al_fc3, 1)  # Resultant shape: [batch_size, 64]

      x_al_out = torch.cat([x_al_1, x_al_2], dim=1)
      x_al_out = torch.cat([x_al_out, x_al_3], dim=1)

      x_al_out = self.avgpool(x_al_out)
      x_al_out = torch.flatten(x_al_out, 1)
      out_al = self.fc(x_al_out)


      return x_gl_1, x_gl_2, x_gl_3, x_al_1, x_al_2, x_al_3, x_gl_fc1, x_gl_fc2, x_gl_fc3, x_al_fc1, x_al_fc2, x_al_fc3, out_gl, out_al
